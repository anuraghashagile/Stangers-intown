...
export const sendOfflineMessage = async (senderId: string, receiverId: string, message: Message) => {
  ...
  // console.log(`[DB] Sending offline message from ${senderId} to ${receiverId}`);
  ...
  if (error) {
    console.error("[DB] Error sending offline message:", error);
    throw error;
  }
};

export const fetchOfflineMessages = async (myPeerId: string) => {
  // console.log(`[DB] Fetching offline messages for ${myPeerId}`);
  const { data, error } = await supabase
  ...
  if (error) {
     console.error("[DB] Error fetching offline messages:", error);
     return [];
  }
  return data || [];
};
...
export const markMessageAsRead = async (messageId: string) => {
  const { error } = await supabase.from('direct_messages').update({ is_read: true }).eq('message_id', messageId);
...
export const sendPersistentGlobalMessage = async (message: Message) => {
  ...
  if (error) {
    console.error("[DB] Failed to send global message. ensure tables exist!", error);
    throw error;
  }
};
...
export const fetchRecentGlobalMessages = async (): Promise<Message[]> => {
  ...
  if (error) {
     console.error("[DB] Fetch Global Chat Error. Run SQL script!", error);
     return [];
  }
  
  if (!data) return [];

  // Data comes in DESC (newest first). We reverse it to ASC (oldest first) for chat UI.
  return data.reverse().map((row: any) => ({
    ...
    senderProfile: row.sender_profile || undefined,
    timestamp: new Date(row.created_at).getTime(),
    type: 'text'
  }));
};
